...
def tela_inicial():
    return [
        [sg.Button('?')],
        [sg.Text('Numero para excluir um pedido de compra'), sg.Input('0872',key='-pesquisa-')],
        [sg.Text('Numero para cancelar uma comprar'), sg.Input('0806107',key='-pesquisa-')],
    ...
    if event == 'OK':
        ...
        if papel == 'personal':
            ...
            while True:
                ...
                if eventos == '?':
                    janela.close()
                    janela = sg.Window('Adicionar tem de validade minima faltande', loja())
                    while True:
